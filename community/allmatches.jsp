...
<html>
	<head>
		...
		<title>BitMatch | Find Match</title>
		...
		<div id="minimum">
			<div class="wrapper" id="alluserdiv">
				...
				<div id="pages">
					...
					<c:forEach var="page" begin="${offset}" end="${fn:replace(((matchList.size()/amount)-(matchList.size()/amount)%1),'.0','') }">
					...
					<c:if test="${offset+2 < fn:replace(((matchList.size()/amount)-(matchList.size()/amount)%1),'.0','')}">
						<a href="/community/allusers.jsp?start=${offset+1}&filter=${amount}">&gt;</a>
						<a href="/community/allusers.jsp?start=${fn:replace(((matchList.size()/amount)-(matchList.size()/amount)%1),'.0','')}&filter=${amount}">&gt; Last</a>
					</c:if>
				</div>
				<div id="users">
